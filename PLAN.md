...
| Phase 2: library and officials | In progress | The library authoring format and infrastructure are implemented, sharing chapters' Markdown/JSON parser via an extracted common core. Six source-verified documents are published: the Declaration of Independence, the Constitution's Preamble and Articles I-VII, Amendments I-XXVII, and — from the Citizen's Almanac — "Patriotic Anthems of the United States," "Patriotic Symbols of the United States" (together completing the Almanac's anthems-and-symbols section), and the first of its seven speeches, Washington's Farewell Address. Six more speeches and four landmark cases are deliberately deferred. A dated snapshot covers four federal offices, state capitals, both senators in every state, and all 435 House members plus D.C.'s delegate; the 119th Congress Census ZIP-to-district crosswalk is bundled; state QIDs enable a live, per-state governor refresh (not bundled — see below). |
...
3. The Citizen's Almanac library documents are underway: "Patriotic Anthems
   ...
   The first of the Almanac's seven speeches, Washington's Farewell Address
   (pages 28-29), is also published; unlike the founding documents, the
   Almanac prints only excerpts of each speech, and this document transcribes
   exactly that selection rather than inventing the unprinted remainder.
   Remaining: six more speeches and the four landmark Supreme Court cases,
   each carrying the required citation.
...
range, and one `{year}` item per subheading) resolved at parse time into
`CategoryNav` — editorial grouping and timeline metadata (Bill of Rights;
Early Amendments; Reconstruction; Progressive Era; Modern) that is never
checked against the source text and never appears inside the transcribed
legal text itself, only in the reader's sidebar, which now renders as a
chronological timeline: one dot per era, each amendment listed under it with
its own ratification year (already stated in the document's own transcribed
ratification notes, e.g. "was ratified February 7, 1795" — repeated in the
front matter only so the sidebar can show it without re-parsing prose).
`ParseLibraryDoc` fails closed if an era's item count doesn't exactly match
its slice of the document's subheadings, so the timeline can't silently
drift out of sync with the document it describes. Each amendment's own
heading became a `###` subheading rather than a top-level `##` heading for
the same reason (the existing flat per-`##`-heading TOC every other document
uses would still list all 27). `amendments.md`'s three-line printed title
splash was also brought in line with declaration.md's and
us-constitution.md's convention — treated purely as page furniture rather
than authored as real headings — a correction that surfaced a genuine
ordering bug in `TestLibrarySourceCoverage` (a footnote-reference digit
glued onto the splash's own first word needed its fix applied before, not
after, the checks that key off the already-fixed text). Each library
reader's `h3` subheading is now a larger serif display treatment with a top
rule and more vertical space, giving every amendment (and every Almanac
song) clearer visual separation without needing an image — none is
available or sourceable for a plain legal-text page.
