...
// splitDocument separates the JSON front matter from the Markdown body shared
// by chapters and library documents, and decodes the front matter into meta.
func splitDocument(r io.Reader, meta any) (body string, err error) {
	b, err := io.ReadAll(r)
	if err != nil {
	...
	if len(parts) != 3 || parts[0] != "" {
		return "", fmt.Errorf("document requires delimited metadata")
	}
	decoder := json.NewDecoder(strings.NewReader(parts[1]))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(meta); err != nil {
		return "", fmt.Errorf("decoding document metadata: %w", err)
	}
	if err := decoder.Decode(new(any)); err != io.EOF {
		return "", fmt.Errorf("document metadata contains trailing data")
	}
	return parts[2], nil
}

// parseBlocks supports the small authored Markdown subset documented in
// data/chapters/README.txt, shared by chapters and library documents.
func parseBlocks(body string, sourceStart, sourceEnd int) ([]Block, error) {
	var blocks []Block
	...
	for i := 0; i < len(lines); i++ {
		...
		if match := pageMarker.FindStringSubmatch(line); match != nil {
			...
			if err != nil || n < sourceStart || n > sourceEnd || n <= page {
				return nil, fmt.Errorf("invalid source page %s", match[1])
			}
			page = n
			blocks = append(blocks, Block{Kind: "page", Page: page, Anchor: fmt.Sprintf("page-%d", page)})
			continue
		}
		if page == 0 {
			return nil, fmt.Errorf("document content requires a source page")
		}
		...
		case line == "```text" || line == ":::sidebar":
			...
			if i == len(lines) || len(text) == 0 {
				return nil, fmt.Errorf("unclosed or empty %s on page %d", block.Kind, page)
			}
		...
		case strings.HasPrefix(line, "!["):
			...
			if match == nil {
				return nil, fmt.Errorf("invalid image reference on page %d", page)
			}
			block.Kind = "image"
			block.Text = match[1]
			...
			for ; i < len(lines) && strings.HasPrefix(strings.TrimSpace(lines[i]), "- "); i++ {
				...
				if strings.HasPrefix(lines[i], "  - ") {
					if len(block.Items) == 0 {
						return nil, fmt.Errorf("nested list without parent on page %d", page)
					}
					j := len(block.Items) - 1
		...
		case strings.HasPrefix(line, "<"), strings.HasPrefix(line, "#"), strings.HasPrefix(line, "```"), strings.HasPrefix(line, ":::"):
			return nil, fmt.Errorf("unsupported markup on page %d: %q", page, line)
		default:
			block.Kind = "paragraph"
			block.Text = line
		}
		if block.Kind == "heading" || block.Kind == "subheading" {
			block.Anchor = fmt.Sprintf("section-%d", len(blocks))
		}
		blocks = append(blocks, block)
	}
	if len(blocks) == 0 {
		return nil, fmt.Errorf("document body is empty")
	}
	return blocks, nil
}
...
func ParseChapter(r io.Reader) (Chapter, error) {
	var chapter Chapter
	body, err := splitDocument(r, &chapter)
	if err != nil {
		return chapter, err
	}
	...
	chapter.Blocks, err = parseBlocks(body, chapter.SourceStart, chapter.SourceEnd)
	if err != nil {
		return chapter, err
	}
	return chapter, nil
